	...
	void Update () {
		...
		if (Input.GetMouseButton(2)) {
			if (Physics.Raycast(cam.transform.position, cam.transform.forward, out hit, 100))
            {
   				...
   				int x0 = (int) (Mathf.Round(hitBlock0.x) - hit.collider.gameObject.transform.position.x);
   				int y0 = (int) (Mathf.Round(hitBlock0.y) - hit.collider.gameObject.transform.position.y);
   				int z0 = (int) (Mathf.Round(hitBlock0.z) - hit.collider.gameObject.transform.position.z);
				...
				if (!(c.chunkData[x0,y0,z0].bType == Block.BlockType.STONE)) {
					...
					c.chunkData[x0,y0,z0].SetType(Block.BlockType.REDSTONE);
					c.DrawChunk();
				}
